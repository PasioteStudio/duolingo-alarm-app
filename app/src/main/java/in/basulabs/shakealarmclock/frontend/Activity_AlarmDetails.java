...
public class Activity_AlarmDetails extends AppCompatActivity implements
		...
		if (savedInstanceState == null) {
			...
			if (Objects.equals(getIntent().getAction(), ACTION_NEW_ALARM)) {
				...
				setVariablesInViewModel(MODE_EXISTING_ALARM,
					...
					data.getString(BUNDLE_KEY_ALARM_MESSAGE),
					data.getString(BUNDLE_KEY_ALARM_DUOLINGO),
	...
	private void setVariablesInViewModel(int mode, int alarmHour, int alarmMinute,
		...
		int alarmVolume,
		@Nullable ArrayList<Integer> repeatDays, @Nullable String alarmMessage,@Nullable String alarmDuolingo,
		...
		viewModel.setAlarmDuolingo(alarmDuolingo);
	...
	//----------------------------------------------------------------------------------------------------
	public String Duo="NINCS KÉSZ";
	private SharedPreferences sharedPref;
	public void Duolingo_APi(Boolean username_too){
		...
		try {
			...
			length=STREAK.getInt("length");
			java.text.DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
			Date yesterday = new Date(System.currentTimeMillis());
			if(END_DATA.equals(dateFormat.format(yesterday))){
		...
		if (username_too){
			...
			Streak.setText(String.valueOf(length)+"🔥");
		}
	}
	/**
	...
	public void onSaveButtonClick() {
		...
		data.putString(BUNDLE_KEY_ALARM_MESSAGE,
			viewModel.getAlarmMessage());
		data.putString(BUNDLE_KEY_ALARM_DUOLINGO,
			viewModel.getAlarmDuolingo());
